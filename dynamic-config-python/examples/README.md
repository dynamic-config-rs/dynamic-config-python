...
| [`10_fastapi_service`](10_fastapi_service.py) | FastAPI: configuration as a dependency in both `async def` and `def` endpoints, the app's lifespan as one `running_async()` block, and a test override (`pip install fastapi httpx`) |
...
| [`13_asyncio_many_files`](13_asyncio_many_files.py) | Three configurations, three files, one loop: `asyncio.gather` to load, a follower each, `configure_executor` for the blocking half — and the same three as a `ConfigGroup` at the end |
...
| [`16_callbacks`](16_callbacks.py) | Every callback shape: `on_reload`, the decorator, `on_change` filters, a scoped guard, handing work to the thread that owns it, `dispatch=` for the same hand-off as a parameter, and an async follower |
...
| [`18_python_remote_source`](18_python_remote_source.py) | A remote store written in Python with a synchronous client: `RemoteSource`, an explicit `refresh_remote()`, the GIL measured across a 200 ms fetch, and a store that starts refusing its credential |
...
| [`23_config_group`](23_config_group.py) | `ConfigGroup`: three configurations initialised, watched and stopped as one block, and one `status()` call for a health endpoint |
...
dynamic-config-py[pydantic]`). All twenty-seven run in CI, because an
